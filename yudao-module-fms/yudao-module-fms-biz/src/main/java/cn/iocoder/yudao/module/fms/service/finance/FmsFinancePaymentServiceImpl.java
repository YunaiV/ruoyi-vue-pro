...
import cn.iocoder.yudao.module.fms.dal.mysql.finance.FmsFinancePaymentItemMapper;
import cn.iocoder.yudao.module.fms.dal.mysql.finance.FmsFinancePaymentMapper;
...
@Service
@Validated
public class FmsFinancePaymentServiceImpl implements FmsFinancePaymentService {

    @Resource
    private FmsFinancePaymentMapper financePaymentMapper;
    @Resource
    private FmsFinancePaymentItemMapper financePaymentItemMapper;
...
//    private ErpPurchaseReturnService purchaseReturnService;
    ...
    public Long createFinancePayment(FmsFinancePaymentSaveReqVO createReqVO) {
        // 1.1 校验订单项的有效性
        List<FmsFinancePaymentItemDO> paymentItems = validateFinancePaymentItems(
...
//        supplierService.validateSupplier(createReqVO.getSupplierId());
        ...
        // 1.5 生成付款单号，并校验唯一性
        String no = noRedisDAO.generate(FmsNoRedisDAO.FINANCE_PAYMENT_NO_PREFIX, FINANCE_PAYMENT_NO_OUT_OF_BOUNDS);
...
//        supplierService.validateSupplier(updateReqVO.getSupplierId());
        ...
        // 2.1 更新付款单
        FmsFinancePaymentDO updateObj = BeanUtils.toBean(updateReqVO, FmsFinancePaymentDO.class);
    ...
    private void calculateTotalPrice(FmsFinancePaymentDO payment, List<FmsFinancePaymentItemDO> paymentItems) {
        payment.setTotalPrice(getSumValue(paymentItems, FmsFinancePaymentItemDO::getPaymentPrice, BigDecimal::add, BigDecimal.ZERO));
    ...
    public void updateFinancePaymentStatus(Long id, Integer status) {
        ...
        int updateCount = financePaymentMapper.updateByIdAndStatus(id, payment.getStatus(),
            new FmsFinancePaymentDO().setStatus(status));
        if (updateCount == 0) {
            throw exception(approve ? FINANCE_PAYMENT_APPROVE_FAIL : FINANCE_PAYMENT_PROCESS_FAIL);
        }
    }

    private List<FmsFinancePaymentItemDO> validateFinancePaymentItems(
        Long supplierId,
        List<FmsFinancePaymentSaveReqVO.Item> list) {
        return convertList(list, o -> BeanUtils.toBean(o, FmsFinancePaymentItemDO.class, item -> {
...
//            }
    ...
    private void updateFinancePaymentItemList(Long id, List<FmsFinancePaymentItemDO> newList) {
        // 第一步，对比新老数据，获得添加、修改、删除的列表
        List<FmsFinancePaymentItemDO> oldList = financePaymentItemMapper.selectListByPaymentId(id);
        List<List<FmsFinancePaymentItemDO>> diffList = diffList(oldList, newList, // id 不同，就认为是不同的记录
        ...
        if (CollUtil.isNotEmpty(diffList.get(2))) {
            financePaymentItemMapper.deleteBatchIds(convertList(diffList.get(2), FmsFinancePaymentItemDO::getId));
        }
    ...
    private void updatePurchasePrice(List<FmsFinancePaymentItemDO> paymentItems) {
...
//                purchaseReturnService.updatePurchaseReturnRefundPrice(paymentItem.getBizId(), totalPaymentPrice.negate());
    ...
    public void deleteFinancePayment(List<Long> ids) {
        // 1. 校验不处于已审批
        List<FmsFinancePaymentDO> payments = financePaymentMapper.selectBatchIds(ids);
        ...
        payments.forEach(payment -> {
            ...
            // 2.2 删除付款单项
            List<FmsFinancePaymentItemDO> paymentItems = financePaymentItemMapper.selectListByPaymentId(payment.getId());
            financePaymentItemMapper.deleteBatchIds(convertSet(paymentItems, FmsFinancePaymentItemDO::getId));
    ...
    private FmsFinancePaymentDO validateFinancePaymentExists(Long id) {
        FmsFinancePaymentDO payment = financePaymentMapper.selectById(id);
        if (payment == null) {
    ...
    @Override
    public FmsFinancePaymentDO getFinancePayment(Long id) {
        return financePaymentMapper.selectById(id);
    }

    @Override
    public PageResult<FmsFinancePaymentDO> getFinancePaymentPage(FmsFinancePaymentPageReqVO pageReqVO) {
    ...
    @Override
    public List<FmsFinancePaymentItemDO> getFinancePaymentItemListByPaymentId(Long paymentId) {
        return financePaymentItemMapper.selectListByPaymentId(paymentId);
    }

    @Override
    public List<FmsFinancePaymentItemDO> getFinancePaymentItemListByPaymentIds(Collection<Long> paymentIds) {
