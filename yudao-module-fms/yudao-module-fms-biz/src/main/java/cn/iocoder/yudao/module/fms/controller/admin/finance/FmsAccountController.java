...
import cn.iocoder.yudao.module.fms.controller.admin.finance.vo.account.FmsAccountRespVO;
...
public class FmsAccountController {
    ...
    public CommonResult<List<FmsAccountRespVO>> getWarehouseSimpleList() {
        List<FmsAccountDO> list = accountService.getAccountListByStatus(CommonStatusEnum.ENABLE.getStatus());
        return success(convertList(list, account -> new FmsAccountRespVO().setId(account.getId())
    ...
    public void exportAccountExcel(@Valid FmsAccountPageReqVO pageReqVO,
        ...
        List<FmsAccountDO> list = accountService.getAccountPage(pageReqVO).getList();
        // 导出 Excel
        ExcelUtils.write(response, "结算账户.xls", "数据", FmsAccountRespVO.class,
            BeanUtils.toBean(list, FmsAccountRespVO.class));
    }
