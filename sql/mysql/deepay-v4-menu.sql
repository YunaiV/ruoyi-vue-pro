...
--
-- 步骤 1：软删除所有旧若依菜单（system_menu）
-- 步骤 2：软删除所有旧角色-菜单绑定（system_role_menu）
-- 步骤 3：插入 Deepay 完整菜单树（ID 10000 起）
-- 步骤 4：将 Deepay 菜单授权给超级管理员角色（role_id=1）
--
-- 执行环境：MySQL 5.7 / 8.0
-- 幂等安全：所有 INSERT 使用 INSERT IGNORE
-- =====================================================================

-- ----------------------------
-- Step 1: 软删除所有现有菜单（清空旧若依/其他模块菜单）
-- ----------------------------
UPDATE `system_menu`
SET `deleted`     = b'1',
    `update_time` = NOW()
...
-- ----------------------------
-- Step 2: 软删除旧角色-菜单绑定（避免失效 menu_id 干扰权限判断）
-- ----------------------------
