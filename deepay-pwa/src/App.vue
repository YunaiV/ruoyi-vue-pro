<template>
  <!-- ══ 管理后台：全屏独立，绕过侧栏布局 ══ -->
  <router-view v-if="route.meta.admin" />

  <!-- ══ 普通前台：侧栏 + 主区 ══ -->
  <div v-else class="gpt-shell" :class="{ 'theme-light': !themeStore.isDark }">
    ...
    <aside class="gpt-sidebar" :class="{ open: mobileSidebarOpen }">
      ...
      <!-- Bottom -->
        <div class="sb-bottom">
          <!-- 管理后台入口 -->
          <button class="sb-admin-btn" @click="navTo('/admin')" title="管理后台">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93a10 10 0 010 14.14M4.93 4.93a10 10 0 000 14.14"/></svg>
            管理后台
          </button>
