# bpm-database Specification

## Purpose

定义 BPM (Business Process Management) 模块的数据库初始化脚本规范,确保数据库表结构与 Java 实体类定义保持一致,支持 MySQL 和 PostgreSQL 数据库,并遵循项目统一的数据库设计规范。

本规范覆盖 BPM 模块的所有核心数据表,包括流程分类、表单定义、流程定义扩展信息、流程表达式、流程监听器、用户组、流程抄送和 OA 请假申请示例表。
## Requirements
