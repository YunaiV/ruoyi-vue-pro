...
public interface ImRtcCallService {
     ...
     * @param roomName LiveKit 房间名；从胶囊条 activeCall 拿
     * @return 通话主表
     */
    ImRtcCallDO joinCall(Long userId, String roomName);
     ...
     * @return 通话主表
     */
    ImRtcCallDO acceptCall(Long userId, String roomName);

    /**
     * 拒绝通话；仅 INVITING 状态可拒；群通话拒绝等同于不参与，房间仍存在
     */
    void rejectCall(Long userId, String roomName);

    /**
     * 取消邀请；主叫在 INVITING 状态主动取消
     */
    void cancelCall(Long userId, String roomName);

    /**
     * 离开通话；RUNNING 状态下离开；私聊任一方离开 = 通话结束；群通话最后一人离开才结束
     */
    void leaveCall(Long userId, String roomName);
     ...
     * @return 通话主表
     */
    ImRtcCallDO refreshCallToken(Long userId, String roomName);
     ...
     * @return 参与者明细列表
     */
    List<ImRtcParticipantDO> getCallParticipants(String callId);
     ...
     * @return JWT 字符串
     */
    String signCallToken(Long userId, String roomName);
