...
public class WmsStockFlowController {
    ...
    public CommonResult<Long> createStockFlow(@Valid @RequestBody WmsStockFlowSaveReqVO createReqVO) {
        return success(stockFlowService.createStockFlow(createReqVO));
    }
    ...
    public CommonResult<WmsStockFlowRespVO> getStockFlow(@RequestParam("id") Long id) {
        ...
        return success(BeanUtils.toBean(stockFlow, WmsStockFlowRespVO.class));
    }
    ...
    public CommonResult<PageResult<WmsStockFlowRespVO>> getStockFlowPage(@Valid WmsStockFlowPageReqVO pageReqVO) {
        PageResult<WmsStockFlowDO> pageResult = stockFlowService.getStockFlowPage(pageReqVO);
        return success(BeanUtils.toBean(pageResult, WmsStockFlowRespVO.class));
    }
    ...
    public void exportStockFlowExcel(@Valid WmsStockFlowPageReqVO pageReqVO,
              HttpServletResponse response) throws IOException {
        pageReqVO.setPageSize(PageParam.PAGE_SIZE_NONE);
        List<WmsStockFlowDO> list = stockFlowService.getStockFlowPage(pageReqVO).getList();
        // 导出 Excel
        ExcelUtils.write(response, "库存流水.xls", "数据", WmsStockFlowRespVO.class,
                        BeanUtils.toBean(list, WmsStockFlowRespVO.class));
    }
