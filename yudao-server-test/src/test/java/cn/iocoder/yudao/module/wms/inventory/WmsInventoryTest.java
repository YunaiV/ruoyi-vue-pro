...
public class WmsInventoryTest extends BaseRestIntegrationTest {
    ...
    private Map<String, WmsStockBinRespVO> updateActualQuantity(WmsInventoryRespVO inventoryRespVO) {
        ...
        for (WmsInventoryBinRespVO binItemVO : inventoryRespVO.getBinItemList()) {
            ...
            CommonResult<List<WmsStockBinRespVO>> stockBinResult = this.wms().stockBinClient().getStockBin(warehouseId, binItemVO.getBinId(), binItemVO.getProductId());
        ...
        this.wms().inventoryClient().updateInventoryActualQuantity(saveReqVOS);
    ...
    private void submit(Long inventoryId) {
        CommonResult<Boolean> result = this.wms().inventoryClient().submit(inventoryId);
    }

    private void agree(Long inventoryId,Map<String, WmsStockBinRespVO> binMapBefore) {
        //
        CommonResult<Boolean> result = this.wms().inventoryClient().agree(inventoryId);
