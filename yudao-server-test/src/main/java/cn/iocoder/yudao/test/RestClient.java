...
public class RestClient {
    ...
    public RestClient(Profile profile,TestRestTemplate testRestTemplate) {
    ...
    public <T> CommonResult<T> get(String url,Object param) {
        url=processUrl(url);
    ...
    public <REQ,RESP>  CommonResult<PageResult<RESP>> getPage(String url, REQ reqVO, Class<RESP> respClass) {
        CommonResult result = get(url,reqVO);
        if(result.isError() && result.getCode()==405) {
            result = post(url,reqVO);
        }
        if(result.isError()) {
            return result;
        }
