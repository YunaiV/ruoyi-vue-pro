...
public enum ErpStorageStatus implements ArrayValuable<Integer> {
    ...
    public static final int[] ARRAYS = Arrays.stream(values()).mapToInt(ErpStorageStatus::getCode).toArray();
    private static final Map<Integer, ErpStorageStatus> STATUS_MAP = new HashMap<>();

    static {
        // 将枚举的状态码作为键，枚举值作为值放入Map中
        for (ErpStorageStatus status : values()) {
    ...
    // 根据状态码获取状态枚举
    public static ErpStorageStatus fromCode(int code) {
        ErpStorageStatus status = STATUS_MAP.get(code);
        if (status == null) {
