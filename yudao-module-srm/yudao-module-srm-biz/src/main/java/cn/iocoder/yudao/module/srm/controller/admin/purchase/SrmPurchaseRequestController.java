...
public class SrmPurchaseRequestController {
    ...
    public void exportPurchaseRequestExcel(@Valid SrmPurchaseRequestPageReqVO pageReqVO,
    ...
    private List<SrmPurchaseRequestRespVO> bindList(List<SrmPurchaseRequestDO> oldList) {
        ...
        // 1.1 申请单-产品项
        List<SrmPurchaseRequestItemsDO> purchaseRequestItemList = srmPurchaseRequestService.getPurchaseRequestItemListByOrderIds(
            convertSet(oldList, SrmPurchaseRequestDO::getId));
        ...
        Map<Long, ErpProductDTO> productMap = erpProductApi.getProductMap(
            convertSet(purchaseRequestItemList, SrmPurchaseRequestItemsDO::getProductId));
        //1.3 获取用户信息
        ...
        Map<Long, ErpWarehouseDTO> warehouseMap = wmsWarehouseApi.getWarehouseMap(
            convertSet(purchaseRequestItemList, SrmPurchaseRequestItemsDO::getWarehouseId)
        );
        //1.4 部门信息
        Map<Long, DeptRespDTO> deptMap = deptApi.getDeptMap(
            convertSet(oldList, SrmPurchaseRequestDO::getApplicationDeptId)
        );
        //1.5 供应商信息
        Map<Long, SrmSupplierDO> supplierMap = srmSupplierService.getSupplierMap(
            convertSet(oldList, SrmPurchaseRequestDO::getSupplierId)
        );
        ...
        return BeanUtils.toBean(oldList, SrmPurchaseRequestRespVO.class, purchaseRequest -> {
            ...
            purchaseRequest.setItems(
                BeanUtils.toBean(purchaseRequestItemMap.get(purchaseRequest.getId()), SrmPurchaseRequestItemRespVO.class,
                    item -> {
            ...
            //订单产品总数
            purchaseRequest.setTotalCount(CollUtil.isEmpty(purchaseRequest.getItems()) ? 0 : purchaseRequest.getItems().stream().mapToInt(SrmPurchaseRequestItemRespVO::getCount).sum());
            //创建者、更新者、审核人、申请人填充
