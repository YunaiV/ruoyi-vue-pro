...
@Slf4j
@Component
public class OffActionImpl implements Action<SrmOffStatus, SrmEventEnum, SrmPurchaseRequestDO> {
    @Resource
    ...
    //校验方法
    public static void validate(SrmOffStatus from, SrmOffStatus to, SrmEventEnum event, SrmPurchaseRequestDO context) {
        //手动关闭+自动关闭事件
    ...
    public void execute(SrmOffStatus from, SrmOffStatus to, SrmEventEnum event, SrmPurchaseRequestDO context) {
        ...
        //联动更新子表关闭状态
        if (event == SrmEventEnum.MANUAL_CLOSE || event == SrmEventEnum.AUTO_CLOSE) {
            List<SrmPurchaseRequestItemsDO> itemsDOS = itemsMapper.selectListByRequestId(context.getId());
