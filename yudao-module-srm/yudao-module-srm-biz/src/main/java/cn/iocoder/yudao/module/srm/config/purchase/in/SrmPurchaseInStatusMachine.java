...
public class SrmPurchaseInStatusMachine {
    ...
    public StateMachine<SrmAuditStatus, SrmEventEnum, SrmPurchaseInAuditReqVO> getPurchaseRequestStateMachine() {
        ...
        // 提交审核
        builder.externalTransitions().fromAmong(SrmAuditStatus.DRAFT, SrmAuditStatus.REVOKED, SrmAuditStatus.REJECTED).to(SrmAuditStatus.PENDING_REVIEW).on(SrmEventEnum.SUBMIT_FOR_REVIEW).perform(inAuditActionImpl);
    ...
    public StateMachine<SrmPaymentStatus, SrmEventEnum, SrmPurchaseInDO> getPurchaseRequestPaymentStateMachine() {
        ...
        //付款金额调整
        builder.externalTransition().from(SrmPaymentStatus.NONE_PAYMENT).to(SrmPaymentStatus.PARTIALLY_PAYMENT).on(SrmEventEnum.PAYMENT_ADJUSTMENT).perform(inPayActionImpl);
        builder.externalTransition().from(SrmPaymentStatus.ALL_PAYMENT).to(SrmPaymentStatus.ALL_PAYMENT).on(SrmEventEnum.PAYMENT_ADJUSTMENT).perform(inPayActionImpl);
        builder.externalTransition().from(SrmPaymentStatus.PARTIALLY_PAYMENT).to(SrmPaymentStatus.ALL_PAYMENT).on(SrmEventEnum.PAYMENT_ADJUSTMENT).perform(inPayActionImpl);
        //付款失败事件
        builder.externalTransition().from(SrmPaymentStatus.NONE_PAYMENT).to(SrmPaymentStatus.PAYMENT_EXCEPTION).on(SrmEventEnum.PAYMENT_EXCEPTION).perform(inPayActionImpl);
        builder.externalTransition().from(SrmPaymentStatus.PARTIALLY_PAYMENT).to(SrmPaymentStatus.PAYMENT_EXCEPTION).on(SrmEventEnum.PAYMENT_EXCEPTION).perform(inPayActionImpl);
        //取消付款
        builder.externalTransition().from(SrmPaymentStatus.ALL_PAYMENT).to(SrmPaymentStatus.NONE_PAYMENT).on(SrmEventEnum.CANCEL_PAYMENT).perform(inPayActionImpl);
