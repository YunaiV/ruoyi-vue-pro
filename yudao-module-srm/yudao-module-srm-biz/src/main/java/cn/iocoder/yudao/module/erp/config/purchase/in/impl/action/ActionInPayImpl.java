...
@Slf4j
@Component
public class ActionInPayImpl implements Action<ErpPaymentStatus, SrmEventEnum, ErpPurchaseInDO> {
    @Resource
    ...
    public void execute(ErpPaymentStatus from, ErpPaymentStatus to, SrmEventEnum event, ErpPurchaseInDO context) {

        ErpPurchaseInDO erpPurchaseInDO = erpPurchaseInMapper.selectById(context.getId());

        // 付款金额调整 - 动态状态
        if (event == SrmEventEnum.PAYMENT_ADJUSTMENT) {
            // 查询当前采购入库单的所有子项
            List<ErpPurchaseInItemDO> itemDOS = mapper.selectListByInId(context.getId());
            ...
            // 遍历所有子项，检查支付状态
            for (ErpPurchaseInItemDO itemDO : itemDOS) {
                int payStatus = itemDO.getPayStatus();

                if (payStatus == ErpPaymentStatus.NONE_PAYMENT.getCode()) {
                    allPaid = false;  // 存在未付款子项
                } else if (payStatus == ErpPaymentStatus.PARTIALLY_PAYMENT.getCode()) {
                    hasPartialPayment = true;  // 存在部分付款子项
                    allUnpaid = false;
                    allPaid = false;
                } else if (payStatus == ErpPaymentStatus.ALL_PAYMENT.getCode()) {
