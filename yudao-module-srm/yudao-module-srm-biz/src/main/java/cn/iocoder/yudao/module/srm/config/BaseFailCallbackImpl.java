...
public class BaseFailCallbackImpl<S, E, C> implements FailCallback<S, E, C> {
    ...
    public void onFail(S sourceState, E event, C context) {
        ...
        String msg = StrUtil.format("无法在({})状态下触发({})事件，上下文：{}", statusDesc, SrmEventEnum.valueOf(event.toString()).getDesc(),
            context.getClass().getName());
        log.warn(msg);
        //        throw new IllegalArgumentException(msg);
        //        throw new ServiceException(msg);
        throw exception(PURCHASE_REQUEST_NOT_EXISTS_BY_STATUS, statusDesc, SrmEventEnum.valueOf(event.toString()).getDesc());
