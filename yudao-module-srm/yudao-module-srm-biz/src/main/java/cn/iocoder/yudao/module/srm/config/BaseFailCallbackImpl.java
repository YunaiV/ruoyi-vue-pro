...
public class BaseFailCallbackImpl<S, E, C> implements FailCallback<S, E, C> {
    ...
    public void onFail(S sourceState, E event, C context) {
        ...
        //        String msg = StrUtil.format("无法在({})状态下触发事件({})，上下文：{}", description, SrmEventEnum.valueOf(event.toString()).getDesc(), JSONUtil.toJsonStr(context));
        String msg = StrUtil.format("无法在({})状态下触发事件({})，上下文：{}", description, SrmEventEnum.valueOf(event.toString()).getDesc(), context.getClass().getName());
        log.warn(msg);
